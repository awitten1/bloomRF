...
void runRangeExperiments(T interval_size, Generator d, QueryGenerator qg) {
  ...
  ExperimentDriver<T, Generator, QueryGenerator> ed64U{
      BloomFilterRFParameters{
          6000000, 0, {10, 8, 4, 4, 4, 4, 4, 3}, {1, 1, 1, 1, 1, 1, 2, 2}},
      d, qg};
...
void runPointExperiments(Generator d, QueryGenerator qg) {
  ...
  ExperimentDriver<T, Generator, QueryGenerator> ed64U{
      BloomFilterRFParameters{
          3200000, 0, {8, 8, 6, 6, 5, 5, 4, 3}, {1, 1, 1, 1, 1, 1, 1, 2}},
      d, qg};
...
int main() {
  ...
  runPointExperiments<uint64_t>(
      [&]() mutable {
        uint64_t val =
            std::round(std::normal_distribution<double>(1e8, 1e7)(gen));
        return val;
      },
      [&]() mutable {
  ...
  runRangeExperiments<uint64_t>(
      ...
      [&]() mutable {
        uint64_t val =
            std::round(std::normal_distribution<double>(1 << 30, 1 << 28)(gen));
        return val;
      },
      [&]() mutable {
        return std::uniform_int_distribution<uint64_t>(
