...
size_t BloomRF<T>::bloomRFHash(T data, size_t i) {
    ...
    auto hash = [=]() mutable {
        data >>= (delta - 1);
        size_t hash1 = CityHash64WithSeed(reinterpret_cast<const char*>(&data), sizeof(data), seed);
        size_t hash2 = CityHash64WithSeed(reinterpret_cast<const char*>(&data), sizeof(data), SEED_GEN_A * seed + SEED_GEN_B);
        return (hash1 + i * hash2 + i * i);
    };
    return (hash() % (size() >> (delta - 1))) 
        << (delta - 1) + 
        (data & (1 << (delta - 1)) - 1);
}
...
void BloomRF<T>::add(T data) {
    for (size_t i = 0; i < hashes; ++i) {
        size_t pos = bloomRFHash(data, i) % size();
